...
export function getCachedCollection(resource) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const data = yield (yield fetch(`https://swapi.dev/api/${resource}`)).json();
        localStorage.setItem(resource, JSON.stringify(data));
        return Promise.resolve(data);
    });
}
;
export function getCachedSingle(resource, id) {
    return __awaiter(this, void 0, void 0, function* () {
        const cache = localStorage.getItem(`${resource}/${id}`);
        if (cache) {
            return JSON.parse(cache);
        }
        const data = yield (yield fetch(`https://swapi.dev/api/${resource}/${id}`)).json();
